---
title: "Data_clearning_Nigeria_endline"
output: html
date: "2024-02-20"
---
...
This script has been used to clean data and calculate key indicators for an IDH Farmfit Case in Nigeria. It has been adapted to include new functions from Akvo's DataKit library. These can be seen in the **Example** sections. Each of these rewrites the code used directly above it, showcasing the process using a specific function.
...
# EXAMPLE: *Load DataKit*

If you are using DataKit for the first time, you will need to download it first from Github.

```{r, eval = TRUE}
# Install devtools if not yet installed
install.packages("devtools")

# Install DataKit from GitHub
devtools::install_github("akvo/data-team-R-utils")
...
If you have already previously used and downloaded DataKit, you can just load it as any other library. If there have been new functions added since your last download, you need to download it again to gain access as the library does not automatically update.
...
```{r}
data_filename <- "GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/2401 Alluvial Nigeria (endline)/DATA_ANALYSIS-Alluvial-rice.xlsx"
survey_filename <- "/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/2401 Alluvial Nigeria (endline)/SURVEY_FORM-Alluvial-Nigeria-Rice.xlsx"
```
...
```{r}
question_library <- read_excel("/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/2401 Alluvial Nigeria (endline)/question library format v4.1.1.xlsx",
                               sheet = "Full Survey", skip = 2) 
vars_transformed <- read_excel("/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/2401 Alluvial Nigeria (endline)/variables with transformation_2.xlsx") 
household_demographics  <- read_excel("/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/2401 Alluvial Nigeria (endline)/household_demographics_var_2.xlsx") 
vars_dashboard <- read_excel("/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/2401 Alluvial Nigeria (endline)/variable names input dashboard_2.xlsx") 
...
outlier_detection_fun <- function(x){
  ifelse(
...
# EXAMPLE: *clean_raw_data*

The first DataKit function we will use in this script is the *clean_raw_data* function. The code below will show the function documentation.

```{r}
?clean_raw_data
```

There might be extra data cleaning steps that you may need to add manually, however using this function saves space and time. The below chunk of code, goes through the exact same procedure as the code above, using the *clean_raw_data* function.

```{r, eval=TRUE}
Data <- clean_raw_data(Data, 
                       irrelevant_columns = c("Display Name","Device identifier",
                                              "Instance","Form version"), 
                       deduplicate_by = NULL) %>%
  select(-contains("--option--")) %>%
  rename_all( funs(gsub("--other--", "_other2", ., perl=T))) %>%
  mutate_at(vars(numerical_columns), funs(as.numeric)) %>%
...
Here we see that in the cleaning summary, the function was not able to exclude farmers that refused to participate, because there are multiple consent columns. We know based on:

```         
Filter skipped: multiple matches for 'accepted to participate'.
```

This then needs to be done manually.
...
# EXAMPLE: *check_cat_variable*

You may want to get more information about categorical variables.

```{r}
?check_cat_variable
```

This is helpful either here with measurement/land units, or later on during data cleaning and analysis to ensure that the variables are not extremely skewed or to just get a quick overview of the situation. The function automatically shows a chart, however here it may not be needed so we can disable it.

```{r}
check_cat_variable(Data, f_unit_land, show_plot = FALSE)
```
...
#        na.rm = TRUE, remove = FALSE, sep = " ") %>%
  ...
  mutate(f_focus_crop_size_acre = case_when(
    ...
    f_unit_land == "poles" ~ f_focus_crop_size / 160,
...
#  mutate(f_size_acre = ifelse(f_unit_land == "poles", f_size / 160, f_size)) %>%
#  mutate(f_focus_crop_size_acre = ifelse(f_unit_land == "hectares", f_focus_crop_size * 2.471, f_focus_crop_size)) %>%
#  mutate(f_focus_crop_size_acre = ifelse(f_unit_land == "poles", f_focus_crop_size / 160, f_focus_crop_size)) %>%
#  # Fill missing values in focus crop size
#  mutate(f_focus_crop_size_acre = na.locf(f_focus_crop_size_acre, na.rm = FALSE)) %>%
...
#  mutate(f_size_hectare = ifelse(f_unit_land == "poles", f_size / 395.4, f_size_acre / 2.471)) %>%
#  mutate(f_focus_crop_size_hectare = ifelse(f_unit_land == "hectares", f_focus_crop_size, f_focus_crop_size_acre / 2.471))%>%
#  mutate(f_focus_crop_size_hectare = ifelse(f_unit_land == "poles", f_focus_crop_size / 395.4 , f_focus_crop_size_acre / 2.471))
```
...
The below code showcases the use of the merge_other function from DataKit. Start by revieving documentation:

```{r}
?merge_other
```

After that we simply run the function.

```{r, eval=TRUE}
Data <- merge_other(Data)
```

The function also produces a warning in case no "other" variable parent is found, which is worth reviewing:

```         
Warning: No matching parent column found for 'pi_location_other'
Warning: No matching parent column found for 'cl_other'
```
...
Data <- Data %>%
  mutate(f_focus_crop_size = ifelse(identifier == "ve59k1d97ere" & f_focus_crop_size == 3, NA,
                          ifelse(identifier == "vmm7rd4n7eev" & f_focus_crop_size == 5, NA,
                          ifelse(identifier == "b02c5b3ewx8n" & f_focus_crop_size == 2.5, NA,
                          ifelse(identifier == "87sdde9wfuf3" & f_focus_crop_size == 2.5, NA,
                          f_focus_crop_size)))))
...
Data <- Data %>%
  mutate(
    ...
    f_focus_measurement_prod = case_when(
      identifier %in% c("m55hnb69ph9n", "0gx7wtexwdgr", "q26qaxu4ry58", "9sebwsm2e1n6", "uvru0gemk3nd", "f4u1qahm2cu4", "c6edt9wqgnwf", "dgq5yc9r41x6", "yxpy10j79mbe", "10vnnjxt07wf", "vym0544mqyec", "fyqkb46tg0t5", "pw21b2qxsxre", "5pk9cwp6pds8", "kwtc0vpvs8mv", "r5wmpht5595x", "fdg28uyv78nh", "6s9udhpm9ceg", "hjmn9h4nxaq9", "v2mpq1wxsntb", "6wn1dvct95qr", "u64bdfy14jt2", "0ke2maw005v9", "rdpm6gsrqt0e", "wycd0wwcjf74", "n3db7c933pdd", "e47rktq93bm1", "xn9pmtkjmak6", "bvw2c8hdtv59", "srhdaggu82g0", "k4pfrgye7c8g") ~ "75 kgs bags", TRUE ~ f_focus_measurement_prod),

    #sold
    f_focus_measurement_sold = case_when(
      identifier %in% c("m55hnb69ph9n", "0gx7wtexwdgr", "q26qaxu4ry58", "9sebwsm2e1n6", "uvru0gemk3nd", "f4u1qahm2cu4", "c6edt9wqgnwf", "dgq5yc9r41x6", "yxpy10j79mbe", "10vnnjxt07wf", "vym0544mqyec", "fyqkb46tg0t5", "pw21b2qxsxre", "5pk9cwp6pds8", "kwtc0vpvs8mv", "r5wmpht5595x", "fdg28uyv78nh", "6s9udhpm9ceg", "hjmn9h4nxaq9", "v2mpq1wxsntb", "6wn1dvct95qr", "u64bdfy14jt2", "0ke2maw005v9", "rdpm6gsrqt0e", "wycd0wwcjf74", "n3db7c933pdd", "e47rktq93bm1", "xn9pmtkjmak6", "bvw2c8hdtv59", "srhdaggu82g0", "k4pfrgye7c8g", "p8pwxqknatct", "vmm7rd4n7eev") ~ "75 kgs bags", TRUE ~ f_focus_measurement_sold),
      #let's harmonise measurement unit 
     f_focus_measurement_sold = ifelse(f_focus_measurement_sold == "75 kilogram bags", "75 kgs bags", f_focus_measurement_sold),

    #own consumption
    f_focus_own_consumption_measurement =ifelse(f_focus_own_consumption_measurement == "kgs", "75 kgs bags",
                                                ifelse(f_focus_own_consumption_measurement == "75 kgs bag", "75 kgs bags",
                                                              f_focus_own_consumption_measurement)),

    #lost
    f_focus_measurement_lost = case_when(
      identifier %in% c("4u4atmh89dak") ~ "75 kgs bags", TRUE ~ f_focus_measurement_lost),
      #let's harmonise measurement unit 
    f_focus_measurement_lost =ifelse(f_focus_measurement_lost == "kgs", "75 kgs bags",
...
Data <- Data %>%
  mutate(f_focus_quant_prod=ifelse(identifier=="ft43c4rch3gn" & f_focus_quant_prod==39000,f_focus_quant_sold,
         ifelse(identifier=="05cmerfvgb4t" & f_focus_quant_prod==100000,NA,f_focus_quant_prod)),
         f_focus_quant_sold = ifelse(identifier %in% c("sn7qb75w8mup", "hj680cyajf55", "9t28pq3ygcbm", "m55hnb69ph9n", "0tdg34f8h405", "8jwktnf9af0a"), f_focus_quant_prod, f_focus_quant_sold ), 
         f_focus_own_consumption=ifelse(identifier=="asqpafquwq22",0,f_focus_own_consumption),
         f_focus_quant_lost=ifelse(identifier=="asqpafquwq22",0,f_focus_quant_lost)
         )
```
...
#some need to be multiplied by 100
Data <- Data %>%
  mutate(f_focus_price = ifelse(f_focus_price >=30 & f_focus_price <=36, f_focus_price * 100, f_focus_price))

#those with a typo, multiply by 10
Data <- Data %>%
  mutate(f_focus_price = ifelse(f_focus_price == 3500 | f_focus_price == 3800, f_focus_price * 10, f_focus_price))

#calculate median
median_price_75kgs<- Data %>%
  filter(f_focus_measurement_sold == "75 kgs bags") %>%
  summarize(median_price_75kgs = median(f_focus_price, na.rm = TRUE)) 


# Replace prices with median for specific identifiers with odd prices
Data <- Data %>%
  mutate(f_focus_price = if_else(identifier %in% c("e4erabp4qstf", "73ueghubc2qg", "hxq6wn72by4f"),
                                 median_price_75kgs$median_price_75kgs, f_focus_price))



# #lets define the id where price need to be replaced by median
# identifiers_to_replace <- c ("e4erabp4qstf")
#   
# #now replace
# Data <- Data %>%
#   mutate(f_focus_price = ifelse(
#     identifier %in% identifiers_to_replace & !is.na(median_price_75kgs$median_price_75kgs),
#     median_price_75kgs$median_price_75kgs,
#     f_focus_price))%>%
  # mutate(f_focus_price = ifelse(f_focus_price==0 & f_focus_quant_sold !=0, median_price_75kgs$median_price_75kgs, 
  #                               ifelse(identifier %in% c("pgq2je5yr7tp","5bhdwdf0x0f7", "t63rf1gs134f", "ajqf4f8c3tnx", "28sj15xb7hua"), median_price_75kgs$median_price_75kgs, f_focus_price)))


##UPDATED CODE
#as there are multiple seasons, farmer who sold at one season, but not in the other, we need to make sure that the first row contains a value for f_focus_price. Therefore, we get the value from the other season when the farmer did sell.
# Replace prices based on specific identifiers
Data <- Data %>%
  mutate(f_focus_price = case_when(
    identifier == "pgq2je5yr7tp" ~ 34000,
    identifier %in% c("5bhdwdf0x0f7", "ajqf4f8c3tnx") ~ 30000,
    identifier == "t63rf1gs134f" ~ 27000,
    identifier == "28sj15xb7hua" ~ 33000,
    TRUE ~ f_focus_price
  ))
```
...
```{r, eval=FALSE}
Data <- Data %>%
  mutate(across(all_of(numerical_columns), outlier_detection))
```
...
# EXAMPLE: *outlier_detection & replace_placeholders*

The next set of functions comes together: The outlier_detetction function and the replace_placeholders function. The outlier detection function is the same as we use and know it, replacing values ±3sd from the mean.

```{r}
?outlier_detection
```

We start off by detecting outliers. We want the replacement to be 9997, so there is no need to further specify as this is the default. Columns that include location, duration and phone numbers are to be excluded from outlier analysis to keep the original information.

```{r, eval=TRUE}
Data <- outlier_detection(Data, exclude_columns = c("duration", "pi_geolocation_latitude", "pi_geolocation_longitude", "pi_geolocation_elevation", "mobile_number_farmer"))
```

Now that outliers have been detected and replaced by 9997 we want to change them to missing values, together with other codes we use to indicate non-response (9999, 9998, 'I don't know' etc.). We do this using the replace_placeholders function.
...
```{r, eval=TRUE}
Data <- replace_placeholders(Data, character_values = c("i don't know", "i prefer not to say"))
```
...
##Measurement unit for production
  mutate(cal_focus_measurement_prod := ifelse(grepl("[[:digit:]]", f_focus_measurement_prod), 
      ...
      ifelse(f_focus_measurement_prod %in%c("kg","kgs"), 1, 0)))
```
...
##Measurement unit for sales focus crop
  mutate(cal_focus_measurement_sold := ifelse(
      ...
      ifelse(f_focus_measurement_sold %in%c("kg","kgs"), 1, 0)))
...
# measurement unit for lost
  mutate(cal_focus_lost_measurement := ifelse(
    ...
    ifelse(f_focus_measurement_lost %in%c("kg","kgs"), 1, 0)))
```
...
# measurement unit for lost
  mutate(cal_focus_measurement_own_consumption := ifelse(
    ...
    ifelse(f_focus_own_consumption_measurement %in%c("kg","kgs"), 1, 0))) 
```

### 2) Quantities produced, sold, lost and own consumption

In this section the quantities produced, sold, lost and own consumption are cleaned. Now that the unit measurements are transformed, the quantities produced, sold, lost and own consumption can be calculated.
...
Data <- Data %>%
  ...
  mutate(
    cal_focus_quant_prod_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season 1  june  wet season season", cal_focus_quant_prod_kg, NA),
    cal_focus_quant_prod_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season 2  december dry season", cal_focus_quant_prod_kg, NA),
    cal_focus_quant_prod_kg_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_prod_kg, NA), #   
    cal_focus_quant_sold_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season 1  june  wet season season", cal_focus_quant_sold_kg, NA),
    cal_focus_quant_sold_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season 2  december dry season", cal_focus_quant_sold_kg, NA),
    cal_focus_quant_sold_kg_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_sold_kg, NA),
#
    cal_focus_quant_own_consumption_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season 1  june  wet season season", cal_focus_measurement_own_consumption, NA),
    cal_focus_quant_own_consumption_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season 2  december dry season", cal_focus_measurement_own_consumption, NA),
...
#
    cal_focus_quant_lost_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season 1  june  wet season season", cal_focus_quant_lost_kg, NA),
    cal_focus_quant_lost_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season 2  december dry season", cal_focus_quant_lost_kg, NA),
...
Data <- Data %>%
  ...
  mutate(
    cal_labour_cost_season_1_dry = ifelse(f_focus_labour_rev_timeperiod == "season 1 june  dry season", cal_labour_cost, NA),
    cal_labour_cost_season_1_wet = ifelse(f_focus_labour_rev_timeperiod == "season 1 june  wet season", cal_labour_cost, NA),
...
group_by(identifier) %>%
  mutate(
    cal_labour_cost_season_1_dry = sum(na.omit(cal_labour_cost_season_1_dry), na.rm = TRUE),
    cal_labour_cost_season_1_wet = sum(na.omit(cal_labour_cost_season_1_wet), na.rm = TRUE),
...
# EXAMPLE: *check_num_variable*

Production cost is a variable that combines three different costs and sometimes causes quite some trouble in analysis. You might want to double check everything went well in your cleaning and calculation by exploring this variable.
...
```{r}
check_num_variable(Data, cal_focus_cost)
```

Perhaps you want a more granular look into production cost. In that case you can adjust the number of bins shown in the histogram.

```{r}
check_num_variable(Data, cal_focus_cost, bins = 50)
```
...
```{r, include = FALSE}
Data <- Data %>%
          rename(location_cascade_region = pi_location_cascade_community_) %>%  ### the variable on the right differs from case to case!!!
          select(
...
```{r}
data_check <- Data %>%
  summarise(Yield = median(cal_focus_productivity_acre, na.rm = TRUE), 
            Income_crop = median(cal_focus_income, na.rm = TRUE), 
            Production_cost = median(cal_focus_cost, na.rm = TRUE), 
            Missing_values = sum(is.na(Data))
            )

write.xlsx(data_check,"Data Check", sheet = 2)
```
...
NB:If the pi\_ file is not available for basline, use the raw data and generate it. here is and example
...
```{r}
baseline_farmers <- read_excel("Pi_alluvial_baseline.xlsx")
